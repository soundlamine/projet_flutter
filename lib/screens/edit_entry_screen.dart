...
class _EditEntryScreenState extends State<EditEntryScreen> {
  ...
  void _initializeFromEntry() {
    ...
    _caloriesController = TextEditingController(text: entry.calories.toStringAsFixed(1));
    _proteinsController = TextEditingController(text: entry.proteins?.toStringAsFixed(1) ?? '');
    _carbsController = TextEditingController(text: entry.carbs?.toStringAsFixed(1) ?? '');
    _fatsController = TextEditingController(text: entry.fats?.toStringAsFixed(1) ?? '');
    _fiberController = TextEditingController(text: entry.fiber?.toStringAsFixed(1) ?? '');
    _sugarController = TextEditingController(text: entry.sugar?.toStringAsFixed(1) ?? '');
    _servingSizeController = TextEditingController(text: entry.servingSize.toStringAsFixed(1));
    _notesController = TextEditingController(text: entry.notes ?? '');

    _selectedCategory = entry.category;
    _selectedMealType = entry.mealType;
    _selectedDate = entry.date;
    _selectedTime = TimeOfDay.fromDateTime(entry.date);
    _enableNotification = entry.notificationEnabled;
  }
  ...
  Future<String?> _scheduleNotification(
    ...
    try {
      ...
      final entryId = 'edit_${widget.entry.id}_${DateTime.now().millisecondsSinceEpoch}';
  ...
  Future<void> _updateEntry() async {
    ...
    try {
      ...
      await _cancelExistingNotification();
      ...
      await calorieProvider.updateEntry(updatedEntry);
          ...
          SnackBar(
            content: Text(
              _enableNotification && notificationId != null
                ? ' Entrée modifiée avec rappel à $formattedTime'
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('Erreur lors de la modification de l\'entrée'),
          backgroundColor: Colors.red,
        ),
      );
    } finally {
  ...
  Future<void> _cancelExistingNotification() async {
    if (widget.entry.notificationId != null) {
      try {
        
        final notificationId = widget.entry.notificationId!.hashCode & 0x7fffffff;
        await _fcmService.cancelScheduledNotification(notificationId);
        print('Ancienne notification annulée: $notificationId');
      } catch (e) {
